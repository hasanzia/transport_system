...
class Trip < ApplicationRecord
  ...
  def completed_trip_should_not_overlap_other_trip_of_same_user_in_same_time
    other_trips_of_same_user = Trip.all_except(self).where(user_id: self.user_id)
    if other_trips_of_same_user.where(" ('#{self.starting_at}' BETWEEN starting_at AND ending_at) OR ( '#{self.ending_at}' BETWEEN starting_at AND ending_at) ").any?
      errors.add(:user_id, "user can't overlap older trip of same user in same time")
    elsif other_trips_of_same_user.where(" (starting_at BETWEEN '#{self.starting_at}' AND '#{self.ending_at}') OR (ending_at BETWEEN '#{self.starting_at}' AND '#{self.ending_at}') ").any?
      errors.add(:user_id, "user can't overlap older trip of same user in same time")
    end
  end

  def completed_trip_should_not_overlap_other_trip_of_same_vehicle_in_same_time
    other_trips_of_same_vehicle = Trip.all_except(self).where(vehicle_id: self.vehicle_id)
    if other_trips_of_same_vehicle.where(" ('#{self.starting_at}' BETWEEN starting_at AND ending_at) OR ( '#{self.ending_at}' BETWEEN starting_at AND ending_at) ").any?
      errors.add(:vehicle_id, "vehicle can't overlap older trip of same vehicle in same time")
    elsif other_trips_of_same_vehicle.where(" (starting_at BETWEEN '#{self.starting_at}' AND '#{self.ending_at}') OR (ending_at BETWEEN '#{self.starting_at}' AND '#{self.ending_at}') ").any?
      errors.add(:vehicle_id, "vehicle can't overlap older trip of same vehicle in same time")
    end
  end
